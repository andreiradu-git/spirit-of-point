  ...
  if (state === "sent") {
    return (
      <div className="max-w-lg mx-auto bg-white/10 backdrop-blur border border-white/20 rounded p-6 text-center">
        <div className="text-lg font-serif mb-2">{t("Message sent")} ✓</div>
        <div className="text-sm opacity-80">{t("Thank you — we'll get back to you shortly.")}</div>
        <button onClick={() => setState("idle")} className="mt-4 text-xs underline">{t("Send another")}</button>
      </div>
    );
  }
  ...
  return (
    <form onSubmit={onSubmit} className="max-w-lg mx-auto text-left space-y-3">
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        <input required name="name" placeholder={t("Your name")} maxLength={200}
          className="bg-white/10 border border-white/30 placeholder:text-white/60 text-white rounded px-3 py-2 text-sm" />
        <input required type="email" name="email" placeholder={t("Email")} maxLength={320}
          className="bg-white/10 border border-white/30 placeholder:text-white/60 text-white rounded px-3 py-2 text-sm" />
      </div>
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        <input name="phone" placeholder={t("Phone (optional)")} maxLength={50}
          className="bg-white/10 border border-white/30 placeholder:text-white/60 text-white rounded px-3 py-2 text-sm" />
        <input name="subject" placeholder={t("Subject (optional)")} maxLength={200}
          className="bg-white/10 border border-white/30 placeholder:text-white/60 text-white rounded px-3 py-2 text-sm" />
      </div>
      <textarea required name="message" placeholder={t("Tell us about your project…")} rows={5} maxLength={5000}
